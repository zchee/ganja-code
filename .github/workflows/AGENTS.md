...
| File | Description |
|------|-------------|
| `ci.yaml` | `lint` (rustfmt, clippy, core-purity) on ubuntu; `deny` (cargo-deny advisories/licenses/bans/sources against `deny.toml`) on ubuntu only, because lockfile analysis cannot differ by OS; `windows-lint` (clippy `--all-targets`, blocking) on windows-2025, the compile loop for `cfg(windows)` code no dev host can cross-build; `test` matrixed over ubuntu, xcode and windows runners — every lane blocking — with the upstream opencode checkout provisioned for the golden differential suite. |
...
- **The upstream spec checkout lands in `upstream/`, not `.omc/`.** `.omc/` is local tooling state that is never committed, so the workflow cannot assume it exists and must not create it. The checkout is `anomalyco/opencode` at `v1.18.13` with `persist-credentials: false`, its `node_modules` are cached everywhere but windows — bun's layout does not survive the cache round-trip there, so that lane installs fresh every run — and installed with `bun install --frozen-lockfile`, and the path is handed to the tests as `GANJA_OPENCODE_DIR`. The golden suite hard-fails when that is missing, so removing any of those steps turns a real comparison into a green run that compared against nothing.
- **The windows matrix entry blocks, and earned it.** It returned observational when the Windows behavior work (ACLs on the auth store and spill files) landed, and flipped after consecutive runs came back green — each red on the way resolved as a product fix, a test fix, or a divergence named in the port plan's ledger. Its `cargo check --workspace` step stays as the separable compile signal, so a red lane answers "stopped building or stopped passing" without a suite log. The job-level `defaults: run: shell: bash` is what keeps the run steps portable (Git Bash on windows); a new step must not assume PowerShell.
