...
- **The core-purity gate is inverted**: `! cargo tree -p ganja-core -e normal | grep -q ratatui`. A plain `grep -c` exits non-zero on *zero* matches, which would fail the build exactly when the core is pure. Its siblings — `! cargo tree -p ganja-tool -e normal | grep -q ganja-core`, and the two bottom-crate leaf checks (`tail -n +2` strips the crate's own header line, which would match) — are inverted for the same reason and assert the other load-bearing directions: nothing beneath the engine reaches the engine, and the bottom crates name nothing else of ours.
