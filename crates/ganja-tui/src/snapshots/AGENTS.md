...
- **A snapshot diff is a question, not a failure.** Read it before accepting: these five cover exactly the states where a rendering regression would otherwise be invisible, so an unexpected change in one is usually a real bug in wrapping, layout or state mapping.
