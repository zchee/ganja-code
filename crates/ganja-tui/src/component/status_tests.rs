...
fn the_model_element_names_the_served_model_only_where_it_differs() {
    ...
    status.set_served_model(Some("claude-opus-5[1m]"));
    assert_eq!(
        rendered(&status, 80),
    ...
    status.set_served_model(Some("default"));
    assert_eq!(
        rendered(&status, 80),
        "Model: default",
...
/// **D556.** [`None`] — what `App::poll_served_model` hands the bar while a
/// delegated child runs — leaves the chosen model drawn alone. The gate itself
/// is the poll's; this pins the bar's half of it.
#[test]
...
/// **D556.** The default bar draws no `model` element at all, so nothing about
/// the served model can reach it — which is what keeps every full-frame
/// snapshot taken over that bar byte-unchanged.
#[test]
...
/// **D556.** The guarded takedown clears the slot only while it still holds
/// the writer's own sentence, and leaves a sentence another writer put there
/// in the meantime standing.
#[test]
fn a_guarded_takedown_leaves_another_writers_notice_standing() {
    const MINE: &str = "one writer's sentence";

    let mut status = roster(&[StatuslineElement::Notice]);
    status.set_notice(Some(MINE.to_owned()));
    status.clear_notice_if(MINE);
