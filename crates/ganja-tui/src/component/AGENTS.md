...
| `status.rs` | The status bar: what the engine is doing (`Activity`), what the session has spent (`Totals`), and the keys that matter. **P13**: three appear-only-while-nonzero segments — `running_jobs` (`"N bash running"`), `running_tasks` (`"N tasks running"`, shown only above one — a lone `task` call is already named by `Activity`), and `queued_dialogs` (`"N dialog(s) queued"`) — all polled off engine state, none tracked as a tally of its own (**D462**). |
