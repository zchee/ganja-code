...
`ratatui` 0.30 + `ratatui-textarea` (editor widget), `crossterm` (feature enablement only), `tokio`, `futures`, `anyhow`, `unicode-width` (the chat viewport measures its own wrap widths rather than leaning on `Paragraph`), `serde_json` (rendering tool metadata), `insta` (dev).
