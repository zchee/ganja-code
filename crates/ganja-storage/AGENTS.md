...
**Every renamer of the database holds that lock**, and the corrupt-store set-aside is the second one. Damage arrives through a different door — `integrity_check` returning a complaint, or a header so broken that `connect` itself errors — but the shape of the race is identical: the verdict is a read on one connection and the rename is a filesystem operation nothing serializes, so two processes meeting one damaged file could each set the other's fresh replacement aside. `set_aside_corrupt` therefore takes `QuarantineLock` and re-probes the path inside it, and a store that has become sound in the meantime is left exactly where it is. With no unlocked renamer left, the inode compare that used to stand behind the pre-UUIDv7 rename retired: a guard behind the lock would only claim to defend a window that no longer opens. A lock that cannot be had is the one case where a damaged store is *not* moved — the caller reopens it and fails loudly, which is recoverable, where renaming a store another process is writing into costs that process its session.
...
- **The quarantine's lock is never removed.** Unlinking `sessions.db.quarantine.lock` would let a later pair of processes race exactly as if there were no lock at all; it is created only when a store is actually about to be set aside — because it predates UUIDv7, or because it will not read — and stays.
- **A new way to rename the database is a new caller of `QuarantineLock`, never a bare `fs::rename`.** The two that exist decide under it and re-probe inside it; a third that did not would reopen the race for both of them, since what makes the others safe is that nothing else can move the file while they hold it.
