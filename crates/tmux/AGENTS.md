...
3. **Every declared flag is a flag, and takes what the builder gives it.** `Entry` carries its command's flags — the letter, and whether a word follows — and each is put to the running tmux's own parser through `bind-key`, which parses a command line and refuses a bad flag *without running it*. tmux's two refusals answer both halves: `unknown flag -X` says the table invented a flag, `-X expects an argument` says a word must follow. All **580** are verified this way. Eight of them (`resize-pane` and `move-pane`'s four directions each) this tmux also takes bare; that allowance is a named table with a reason per row, and each row still has to prove tmux reads the word after it as the flag's argument.

The first two assertions are deliberately **one-way** — installed ⊆ claimed, never the reverse. The tables are written against a newer tmux than the oldest they are meant to serve, so a register entry this tmux has never heard of is *reported* rather than failed; an older tmux must not fail this suite for having fewer commands. A misspelling in either table is still caught, and by the same assertion: the correctly spelled command then appears in neither table.

The third asks in the same direction and fails in the other: a declared flag this tmux refuses is a **hard failure**, because it is a method whose argv tmux answers with `unknown flag` — where a missing *command* is only a builder that tmux cannot reach. A flag this tmux documents and no builder declares is reported by name, never failed, and the report asks the parser which kind it is: today it names `server-access -t`, documented in the usage string and refused by the parser, which is the divergence `sessions.rs` argues in prose.
