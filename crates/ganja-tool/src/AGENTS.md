...
| `skill.rs` | `skill` — a `SKILL.md`'s frontmatter, the discovery walk that finds them (`~/.claude/skills`, `~/.agents/skills`, the same names up to the project root, `skill/`+`skills/` beside a config file, and whatever `skills.paths` added), and the tool that loads one into the conversation. `Roots` is a value so a fixture can hold roots that reach nothing; the same value composes the system prompt's `<available_skills>`, which is what keeps the offered list and the loadable list identical. |
