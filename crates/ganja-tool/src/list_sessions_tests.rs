...
use crate::{Tool as _, ToolCtx, ToolError, registry, socket};
...
fn render_lists_teammates_with_the_lead_marked() {
    ...
    assert!(output.contains(&format!("team-lead: runs the team ({LEAD_MARK})")), "{output}");
    assert!(output.contains(&format!("worker-1: {NO_TEAMMATE_DESCRIPTION}")), "{output}");
}
...
fn render_never_puts_the_unverified_label_on_a_teammate_row() {
    let teammates = vec![Peer { name: "worker-1".to_owned(), description: None, lead: false }];
...
fn render_live_session_rows_carry_every_field_and_the_unverified_label() {
    ...
    assert!(output.contains("uds:/tmp/ganja-501/0198c1a2.sock"), "{output}");
}
...
fn render_neutralizes_a_live_sessions_self_written_name_and_cwd() {
    ...
    assert!(!output.contains("evil\u{7}<script>"), "the raw bytes never reach the model: {output}");
    assert!(!output.contains("/work/<injected>"), "the raw cwd never reaches the model: {output}");
...
async fn the_tool_excludes_itself_and_staleness_and_disambiguates_duplicates_by_stem() {
    ...
    let _own_lock = write_live(dir.path(), "0198c1a2", &record("self", own_session, "/work/self"));
    ...
    registry::write(
        dir.path(),
        "0499f4d5",
        &record("ghost", "0499f4d5-0000-7000-8000-000000000004", "/work/ghost"),
    )
    ...
    let result = tool.run(serde_json::json!({}), &ctx).await.expect("the tool answers");
    ...
    assert!(!result.output.contains("ghost"), "a stale record never appears: {}", result.output);
    ...
    assert!(result.output.contains(TEAMMATES_HEADER), "{}", result.output);
...
async fn the_tool_answers_with_no_postbox_at_all() {
    ...
    assert!(!result.output.contains(TEAMMATES_HEADER), "{}", result.output);
    assert!(result.output.contains(NO_LIVE_SESSIONS), "{}", result.output);
}
...
fn the_description_states_the_same_uid_self_asserted_posture() {
    ...
    assert!(described.contains("nothing here verifies it"), "{described}");
}
