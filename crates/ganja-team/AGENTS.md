...
`cargo nextest run -p ganja-team`, and the workspace gates from the repository root. `tests/claude_format_interop.rs` hard-fails when `tests/fixtures/session-<id>/` is present and skips with a named reason when it is not — the fixtures are committed, so in this tree it runs.
