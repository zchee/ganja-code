...
| `watcher.rs` | The stale-read watcher against a real `notify` watch over a real directory: a file edited from outside is named to the model and refused afterwards even once its stamp is put back (so what refuses is the watcher's verdict and not a comparison), and a tool's own writes are not condemned by the events they cause. Both wait on the watcher's own queue — a stamp comparison would answer the same on a platform where watching does nothing. |
