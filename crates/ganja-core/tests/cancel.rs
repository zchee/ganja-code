...
use ganja_core::{Command, Engine, Event, FinishReason, provider::FakeProvider};
...
async fn cancelling_mid_stream_finishes_the_turn_inside_the_budget() {
    ...
    for _ in 0..WARMUP_FRAGMENTS {
        assert!(matches!(events.next().await, Some(Event::TextDelta { .. })));
    }
    ...
    let reason = loop {
        match events.next().await {
            ...
            Some(Event::TurnFinished { reason }) => break reason,
