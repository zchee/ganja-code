...
//! and not because of a timeout: Claude Code forbids it. The 2.1.233 binary
//! composes the Agent tool's own description with
//!
//! ```text
//! qb() ? "\n- `name` is unavailable here — teammates cannot spawn teammates." : ""
//!
//! function qb(){ if(IU()) return !0;
//!   return !!(X7.dynamicTeamContext?.agentId && X7.dynamicTeamContext?.teamName) }
//! ```
//!
//! and `qb()` is true for exactly a process launched with `--agent-id` **and**
//! `--team-name` — which is [`claude::ClaudePane`]'s launch line, every time. A
//! pane teammate therefore has no `name` parameter to spawn *with*, only
//! anonymous subagents that never earn a member record, so no second member was
//! ever going to appear in the team file. The leg is gone rather than given a
//! longer timeout.
//!
...
//! binary that refuses the witness contains the registration itself. See
//! [`ganja_team::COLLISION_SEPARATOR`], which carries the carved snippet.
