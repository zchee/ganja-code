        ...
        async move {
            loop {
                ...
                let next = {
                    ...
                    if remaining.len() > 1 { remaining.remove(0) } else { remaining[0].clone() }
                };
...
async fn a_refused_request_is_retried_and_then_reported() {
    ...
    let (url, served) =
        serve(vec![refused.clone(), refused.clone(), answered, refused.clone()]).await;
    ...
    assert!(
        catalog::refresh(true).await.expect("the third attempt wins"),
    ...
    assert!(catalog::model("fixture-only").is_some(), "a failed refresh must not empty the table");
}
