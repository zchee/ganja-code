...
/// **AC-22.** A spawn asking to bypass is refused by name: a teammate on
/// another vendor's CLI runs at one pinned posture and asks nobody, so there
/// is no dialog for a bypass to skip.
///
/// Sharper here than on the other two backends, and worth its own sentence:
/// agy's pinned posture is the weakest of the three, so "bypass" would be
/// asking for more than a grant that already has no filesystem bound — which
/// is nothing this build knows how to give and everything a person would
/// wrongly assume it had.
#[tokio::test]
async fn an_agy_spawn_asking_to_bypass_is_refused_by_name() {
    let home = ganja_testkit::temp_dir();
    let cli = FakeAgy::install(Mode::Answer);
    let (registry, door, ..) = lead(home.path(), &cli);
    ...
    let refusal = door
        ...
        .await
        .expect_err("a shim teammate has no escalation tier");

    assert!(refusal.reason.contains(shim::REFUSED_BYPASS), "{refusal:?}");
    assert!(
        cli.received().is_empty(),
        "and nothing was started before the refusal: {:?}",
        cli.received()
    );
    // The bypass clause is backend-independent, so it answers before the
    // backend is reached — fail-closed and intended. As of Dv-7 **two gate
    // clauses answer on this path rather than one**, since agy now has a
    // posture to disclose and the foreign clause answers too — but they are
    // still one dialog, because `SpawnGate::action()` takes the strongest of
    // what was asked rather than opening a window per clause.
    assert_eq!(
        spawn_asks.asked().len(),
        1,
        "one spawn, one dialog: {:?}",
        spawn_asks.asked()
    );
