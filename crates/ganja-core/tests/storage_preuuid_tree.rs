...
fn a_legacy_file_tree_with_old_ids_is_set_aside_once_not_twice() {
    ...
    fs::write(
        &file,
        serde_json::to_vec(&info).expect("the record encodes"),
    )
    ...
    assert!(
        storage
            .list_sessions()
            .expect("the store opens rather than failing")
            .is_empty(),
    ...
    let aside: Vec<&String> = listing
        .iter()
        .filter(|entry| entry.starts_with("storage.preuuid-"))
        .collect();
    ...
    assert!(
        !listing
            .iter()
            .any(|entry| entry.starts_with("storage.migrated-")),
    ...
    assert!(
        database.is_file(),
        "the empty store that replaces the tree is created"
    );
    let name = database
        .file_name()
