...
Sources of assistant text. A provider turns a `ChatRequest` into a stream of `ProviderEvent`s; the engine maps those onto the protocol frontends see. Five ship: a fake one for demos and tests, Anthropic Messages, OpenAI-compatible chat completions, grok — which is xAI's endpoint speaking that same API, so it is a base URL and a credential source over `openai.rs` rather than a second wire — and the Responses API a ChatGPT subscription answers on, which really is a second wire. Every HTTP provider shares the same shape — build a request, retry it while it has not started answering, split the `text/event-stream` body into frames, map those onto events — so everything except the mapping lives in `mod.rs`.
...
Mapping is proved twice: unit tests here, and a suite serving recorded transcripts over a real loopback socket — `../../tests/http.rs` for the two key-authenticated wires, `../../tests/responses_wire.rs` for the subscription one. Fixtures live in `../../tests/fixtures/*.sse` and cover the cases that matter — happy path, truncated stream, mid-stream error, malformed frame, interleaved tool calls, a tool call cut short. A new provider or a new frame shape needs proving over a socket as well as in a unit test, whether the transcript lives in a fixture file or beside the suite that serves it.
