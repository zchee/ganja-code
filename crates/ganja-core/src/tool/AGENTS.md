...
| File | Description |
|------|-------------|
| `mod.rs` | `Tool` trait, `ToolCtx`, `ToolOutput`, `ToolError`, `ToolDefinition`, `Registry`, `FileTimes` (the read-before-write log, including the `Stale` state `../watch.rs` sets and the queue of files still to be named to the model), and `is_credential_store` (the guard `read` and `grep` consult). |
