...
| File | Description |
|------|-------------|
| `mod.rs` | `Tool` trait, `ToolCtx`, `ToolOutput`, `ToolError`, `ToolDefinition`, `Registry`, `FileTimes` (the read-before-write log, including the `Stale` state `../watch.rs` sets, the queue of files still to be named to the model, and the announcer a watcher subscribes to so it can watch the directory each read lands in), and `is_credential_store` (the guard `read` and `grep` consult). |
