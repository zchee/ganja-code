...
| `permission.rs` | Decides which tool calls run unasked, which wait for the user, and which are refused outright (`deny`); layers builtin defaults < agent rules < config rules < stored "always" answers, last match winning; persists "always" answers per project. Also owns `PermissionConfig`/`RuleSet` — the shape a config file's `permission` block parses into, key order intact — so a document decodes straight into the types this module evaluates. |
...
| `watch.rs` | The stale-read watcher: a **non-recursive** `notify` watch on the directory holding each file as the read log records it — never a recursive watch on the project root, which on Linux is a synchronous walk of the whole tree and blocked startup (D171, amending R11). Events are narrowed to paths already in the read log and each is stat'd against the stamp that read recorded. A file that moved is marked `Stale` in `FileTimes` — so `write`/`edit` refuse it — and queued, to be named to the model as one synthetic user part at the top of the next turn that asks it anything. A deliberate extension: upstream watches nothing by default and surfaces nothing to the model. Nothing here can fail a turn; a backend that will not start, or a directory that will not register, is one warning. |
...
- **An MCP tool asks by default.** `permission.rs` decides it by **prefix** (`MCP_PREFIX`), not by name, because the names are not known until a server has been asked. Do not fold this into `ASK_BY_DEFAULT` — that list is names, and this is a shape.
