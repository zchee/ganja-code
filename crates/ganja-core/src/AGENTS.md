...
| `watch.rs` | The stale-read watcher: `notify` on the project root, events narrowed to paths already in the read log, each one stat'd against the stamp that read recorded. A file that moved is marked `Stale` in `FileTimes` — so `write`/`edit` refuse it — and queued, to be named to the model as one synthetic user part at the top of the next turn that asks it anything. A deliberate extension: upstream watches nothing by default and surfaces nothing to the model. Nothing here can fail a turn; a watcher that will not start is one warning. |
