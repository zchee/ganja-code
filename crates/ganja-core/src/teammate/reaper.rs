...
//! record's own **`agentId`** — `<name>@<team>`, unique to this team — must
//! appear in the command line of the pane's first process. That process *is*
//! the teammate: [`crate::teammate::pane`] types `exec` into the shell tmux
//! forked, so `#{pane_pid}` keeps naming the binary that replaced it, and both
//! pane backends carry `--agent-id` on that command line (§4.1). A pane that
//! cannot show the agent id is not this teammate's, whatever id it wears, and
//! is left alone. Two properties fall out of that and are worth naming: the
//! check survives a reboot, since it asks what is running rather than what once
//! was; and a record naming the **lead's own pane** cannot kill it, because the
//! lead's process carries no `--agent-id`.
//!
...
//! document's `leadSessionId` is not this lead's session. The guard is what
//! keeps the shared fallback team (`default`, which every session with a
//! pre-UUIDv7 id joins) from letting one live lead reap another live lead's
//! teammates. Within a team that *is* this lead's, a pane member found before
//! this lead has spawned anything cannot be this lead's own, by construction:
//! the sweep runs at startup, ahead of every spawn.
//!
...
//! `--continue` on an already-open session carries everywhere else.
//!
...
pub async fn sweep_on(registry: &TeammateRegistry, server: &Server) -> Swept {
    ...
    for (name, agent_id, pane_id) in recorded {
        ...
        let witness = match pane {
            Some(pane) => witnessed(pane, &agent_id).await,
            None => None,
        };
...
/// Whether `pane`'s first process is the teammate `agent_id` names, or [`None`]
/// when its command line could not be read.
///
/// The cold-start half of D506's identity. `ps` failing — the process gone
/// between the listing and the question, a machine that answers nothing — is
/// deliberately not "no": a pane nothing is known about is left alone.
async fn witnessed(pane: &Pane, agent_id: &str) -> Option<bool> {
    let argv = argv_of(&pane.birth).await?;

    Some(argv.contains(agent_id))
}
...
mod tests {
    ...
    use super::{Fate, Pane, argv_of, forget, verdict};
