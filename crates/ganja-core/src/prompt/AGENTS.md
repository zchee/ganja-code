...
Prompt texts compiled in with `include_str!`. Nothing here is code. Seven are upstream's — four of them forked to speak in ganja's own name (**D522**) — and six have no upstream lineage at all: `/team`'s template and the five role prompts it routes to (**D547**), written from this port's own behavior specification.
...
| `team.txt` | What `/team` sends (**D547**). `${state}` and `${handoffs}` are filled at roster build with this project's two pipeline directories — `replace`, not `replacen`, because the handoffs one appears twice — and `$ARGUMENTS` with whatever the user typed after the name. `${session}` is the third placeholder and the only one filled at **expansion** rather than at roster build, since one roster serves every session a process opens; it goes in **last**, after `$ARGUMENTS`, the `` !`cmd` `` pass and mention resolution, so a session id can never be read as an argument, run as a command or attached as a path. Ganja's own prose: the pipeline's shape is modelled on oh-my-claudecode's team skill, read as a behavioral reference, and no sentence is taken from it. |
| `analyst.txt`, `executor.txt`, `verifier.txt`, `critic.txt`, `debugger.txt` | The five `/team` roles' own system prompts (**D547**), likewise ganja's own. Each says what the role *is* and nothing about how a team coordinates — the stages, the shared list, the claim discipline and the shutdown are `team.txt`'s to state, and a copy here would be a second place for them to drift. |
