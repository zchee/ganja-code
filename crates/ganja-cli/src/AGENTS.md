...
- **Which login runs is decided in one place, in this order**: `--key`, then `--method`, then "standard input that is not a terminal is a key" (`pass show … | ganja auth login` predates every flow here), then a provider's only login, then a menu. ChatGPT and grok reach the menu — each has a browser login and a device login, and nothing here can tell whether there is a browser on *this* machine; Anthropic and Copilot have one login worth offering and never ask. The menu's words are upstream's own labels, because they are what somebody recognises from having read its documentation.
