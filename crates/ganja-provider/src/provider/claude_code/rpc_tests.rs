...
fn initialize_is_answered_with_the_clis_own_protocol_version_echoed() {
    ...
    let Answer::Reply(sent) = answer(&asked, &roster()) else {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "result": {
                ...
                "serverInfo": {"name": "ganja", "version": env!("CARGO_PKG_VERSION")},
            },
        })
    );
}
...
fn the_server_declares_a_roster_that_can_change_and_announces_it_without_an_id() {
    let Answer::Reply(sent) = answer(&json!({"method": "initialize", "id": 0}), &roster()) else {
        panic!("initialize is answered here");
    };
...
fn a_notification_is_answered_with_the_empty_success() {
    assert_eq!(
        answer(&json!({"method": "notifications/initialized", "jsonrpc": "2.0"}), &roster()),
        Answer::Empty
    );
}
...
fn the_roster_is_declared_under_the_registrys_own_bare_names() {
    let Answer::Reply(sent) = answer(&json!({"method": "tools/list", "id": 1}), &roster()) else {
...
fn the_roster_keeps_the_order_the_engine_advertised() {
    ...
    let Answer::Reply(sent) = answer(&json!({"method": "tools/list", "id": 1}), &tools) else {
        panic!("tools/list is answered here");
    };
...
fn ping_is_answered_with_the_empty_result() {
    let Answer::Reply(sent) = answer(&json!({"method": "ping", "id": 3}), &roster()) else {
...
fn a_method_this_server_does_not_have_is_refused_naming_it() {
    let Answer::Reply(sent) = answer(&json!({"method": "resources/list", "id": 4}), &roster())
    else {
