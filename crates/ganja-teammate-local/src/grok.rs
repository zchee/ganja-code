...
//! all [`crate::shim`]'s, shared with the other CLIs.
//!
...
//! is running — and returned as [`Reply::session`](crate::shim::Reply::session), which
//! [`crate::shim::ShimRunner`] stores exactly as it stores codex's
...
//! shim classifies a non-zero exit as [`Failure::Exit`](crate::shim::Failure::Exit)
...
//!
//! The duplication against [`ganja_core::provider`]'s Messages decoder is
...
use crate::readback;
use crate::shim::{Door, Driver, Reply, Shape, Turn};
