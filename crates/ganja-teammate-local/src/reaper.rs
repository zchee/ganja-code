...
//! [`ganja_core::teammate::TeammateRegistry`] and re-checked inside
//! [`crate::tmux::Server::kill`]. This module cannot have that pair:
...
//! [`crate::pane::AGENT_ID`] with the record's own `agentId`
//! (`<name>@<team>`) **and** [`crate::pane::PARENT_SESSION_ID`] with
//! this lead's session — as flag-and-value pairs, not as substrings anywhere in
//! the line. That process *is* the teammate:
//! [`crate::pane`] types `exec` into the shell tmux forked, so
...
//!   *file*, and [`ganja_core::teammate::TeammateRegistry`]'s record write never
...
//! holding [`crate::pane::SHELL`] and carries no agent id, so a sweep
...
use crate::pane::{AGENT_ID, PARENT_SESSION_ID};
use crate::shim::records::{self, Started, Unreadable};
use crate::shim::signal_group;
use crate::tmux::{Killed, Server};
...
/// one against the live one, in [`crate::tmux::Server::kill`]. Read
/// [`crate::tmux`]'s own module doc for why the second half is a pid.
...
/// session would mark a **live** co-tenant's members dead, so the write bails
/// on exactly the check `sweep_on` makes (reaper.rs's own `lead_session_id`
/// clause) before it touches anything — inside
/// [`TeammateRegistry::mark_records_inactive`] since **Dv-13**, because the
/// check has to be read under the same lock as the write it guards.
///
...
/// members retires nothing and says so by answering empty. That assertion also
/// lives in [`TeammateRegistry::mark_records_inactive`] rather than here — it
/// is that method's safety property, not this caller's discipline.
///
/// # What is this function's, and what is the registry's
///
/// Everything above about *which* rows are a previous lead's shim members —
/// the `backendType` reading, and the [`ShimCli`] table that knows the three
/// names — is this crate's, because the engine has no business knowing what a
/// foreign CLI is called. Everything about *the document* — one hold of the
/// team file lock around read, mutate and write, so a concurrent `record`
/// cannot be written back over — is the registry's, and reaching it takes one
/// call with a predicate rather than five public items and a lock held from
/// out here (**Dv-13**).
///
...
pub async fn retire_shim_records(registry: &TeammateRegistry) -> Vec<String> {
    let marked = registry
        .mark_records_inactive(|member| {
            // `backendType`, never `surface()` — see this function's own doc
            // for why the read that looks more natural cannot answer this.
            member.backend_type.as_deref().and_then(ShimCli::read).is_some()
        })
        .await;
    ...
    match marked {
        ...
        Err(error) => {
            // Read or write — the error says which, and neither is worth a
            // sentence of its own here: the outcome is the same row, still
            // claiming to be running, met again at the next startup.
            tracing::warn!(
                %error,
