...
//! on, and the one every spawn door reaches since P28: [`crate::shim`]
...
//! byte for byte. [`crate::tmux::Server::paste_submit`] is that
...
//! `--agent-id`/`--parent-session-id` pair, so [`crate::reaper`]'s
...
/// (**D514**): the pane channel of [`ganja_core::teammate::preamble::frame`].
///
...
/// [`posture_line`](ganja_core::teammate::posture_line)'s reason and as an
...
/// [`surface_line`](ganja_core::teammate::TeammateBackend::surface_line) hands it
...
/// [`crate::tmux::Server::paste`]), and tmux does not filter the
...
pub struct ShimTui {
    ...
    /// [`crate::pane`]'s own value types, so no backend names a
    /// config type.
...
///
/// The same one-task shape [`crate::shim::ShimMember`] has, for the
...
impl TuiMember {
    ...
    #[must_use]
    pub fn new(pane: Arc<TuiPane>, spec: SpawnSpec, lent: ganja_core::teammate::Lent) -> Self {
        Self {
            pane,
            spec,
            lent,
...
impl TuiRunner {
    ...
    async fn relay(&self) {
        ...
        let Some(path) = transcript else {
            let mark = ganja_core::teammate::preamble::opening(
                ganja_core::teammate::preamble::Names::of(&self.spec),
            );
            let cwd = self.spec.cwd.clone();
            let found = ganja_core::teammate::blocking_io(move || {
    ...
    /// submits ([`Server::paste_submit`](crate::tmux::Server::paste_submit)),
    /// one that never did is pasted only ([`Server::paste`](crate::tmux::Server::paste)),
    ...
    fn remember(&self, line: String) {
        ganja_core::teammate::push_recent(&self.recent, line);
    }
...
async fn carried(
    ...
    ganja_core::teammate::blocking_io(move || {
