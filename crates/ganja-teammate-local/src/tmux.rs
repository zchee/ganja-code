...
//! two backends that do ([`crate::pane`], [`crate::claude`])
...
//! `argv` handed to [`Server::split`](crate::tmux::Server::split)
//! must be at least two words — [`crate::pane::SHELL`] is two for
//! that reason alone.
//!
...
//! [`Server::paste_submit`](crate::tmux::Server::paste_submit) is
...
//! [`Server::capture`](crate::tmux::Server::capture) reads what the
//! pane shows — a readiness poll's source, and a dead pane's last words' —
//! and [`Server::remain_on_exit`](crate::tmux::Server::remain_on_exit)
...
//! of the pane's identity with it. [`Server::panes`](crate::tmux::Server::panes)
...
///
/// This is the mechanism half of **D502** (minted in [`crate::pane`]):
...
/// [`crate::reaper`] gives about panes generally: what is on the
...
pub enum Placement {
    ...
    /// says otherwise ([`crate::pane::PaneShare`]). tmux's `-l`
...
impl Server {
    ...
    /// backends the way [`crate::reaper`]'s covers the one it reaps.
    ...
    /// group is signalled (`crate::shim::signal_group`, the
    ...
    /// [`Pane::birth`](crate::reaper::Pane) is the pid as tmux
    ...
    /// ([`crate::shim_tui`]) reads it once before the launch line
    ...
    /// ([`crate::shim_tui`]) pastes *without* submitting, because an
