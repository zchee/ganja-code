...
//! all [`crate::shim`]'s, shared with the other two CLIs.
//!
...
//!
//! The read scope is why [`ganja_core::teammate::posture_line`]'s codex arm says
...
//!
//! One addition to [`crate::shim::CARRIED`], and it is required
...
use crate::readback;
use crate::shim::{Door, Driver, Launch, Reply, Shape, Turn};
...
/// them — [`crate::tmux::launch_line`]'s job, not this table's.
...
/// mail. [`crate::shim`]'s per-turn deadline does not cover this: it
...
/// The reader [`crate::readback::of`] hands out for this CLI.
...
impl Transcript {
    ...
    fn sessions() -> Option<PathBuf> {
        readback::home_sessions(HOME_ENV, ".codex")
    }
