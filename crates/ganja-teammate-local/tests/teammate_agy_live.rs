...
//!
//! ```sh
//! GANJA_LIVE_TEST=1 cargo test -p ganja-teammate-local --test teammate_agy_live -- --ignored --nocapture
//! ```
//!
...
use ganja_teammate_local::agy::Agy;
use ganja_teammate_local::shim::{self, Driver as _, Turn};
...
async fn a_real_agy_teammate_takes_two_turns_on_one_child_and_writes_where_it_was_told() {
    ...
    let composed = ganja_teammate_local::agy::print_timeout(shim::AGY_TURN_TIMEOUT);
