    ...
    for other in [MemberBackend::Codex, MemberBackend::Agy, MemberBackend::Grok] {
        backends = backends.with(if other == backend {
            ...
            harmless(match other {
                MemberBackend::Codex => Arc::new(ganja_teammate_local::codex::Codex::new()),
                MemberBackend::Agy => Arc::new(ganja_teammate_local::agy::Agy::new()),
                _ => Arc::new(ganja_teammate_local::grok::Grok::new()),
            })
        });
    }
...
/// **real** [`ganja_teammate_local::codex::Codex`] driver, so it cannot be told
...
/// [`ganja_teammate_local::grok::Grok`] driver, so the argv is that vendor's
...
/// [`ganja_teammate_local::agy::Agy`] driver, so the argv is that vendor's and
