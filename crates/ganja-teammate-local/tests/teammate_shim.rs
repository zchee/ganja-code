...
async fn one_pass_sorts_its_inbox_into_turns_and_drops() {
    ...
    use ganja_teammate_local::shim::{Lent, ShimBackend, ShimRunner};
...
async fn a_restarted_lead_marks_a_previous_leads_shim_members_inactive() {
    ...
    let retired = ganja_teammate_local::reaper::retire_shim_records(&restarted).await;
    ...
    assert!(ganja_teammate_local::reaper::retire_shim_records(&restarted).await.is_empty());
...
async fn another_leads_shim_members_are_never_retired() {
    ...
    let retired = ganja_teammate_local::reaper::retire_shim_records(&other).await;
