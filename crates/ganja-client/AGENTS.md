...
| `src/lib.rs` | `Client`: `health`, `create_session`, `sessions`, `prompt`, `events`, `permissions`, `reply_permission`, with `Credentials` (Basic, `Debug` redacted by hand) and `ClientError`. Also the declared bodies — `Health`, `SessionRow` (partial on purpose), `PendingPermission` (whole and closed), `Prompt` — and `Events`, the typed stream. |
