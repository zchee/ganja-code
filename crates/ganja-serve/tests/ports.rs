...
use support::{base_url, engine, loopback_config, tcp};

/// The port `handle` is bound on — these suites bind TCP, so there is one.
fn port(handle: &ganja_serve::Handle) -> u16 {
    handle.address().tcp().expect("a tcp server").port()
}
...
async fn an_explicit_port_that_is_taken_is_refused_rather_than_replaced() {
    ...
    assert!(
        matches!(
            refused,
            Err(ServeError::Bind { ref address, .. })
                if address.tcp().is_some_and(|address| address.port() == taken)
        ),
