...
async fn socket_server(engine: &Arc<Engine>, path: &std::path::Path) -> ganja_serve::Handle {
    ganja_serve::serve(Arc::clone(engine), with_listen(Listen::Unix { path: path.to_owned() }))
        .await
...
async fn a_policy_refused_post_answers_byte_identically_to_an_accepted_one() {
    ...
    let (refusing, refusing_registry) =
        led_engine(refusing_home.path(), Some((InboundPolicy::Refuse, PolicySource::Global)));
    ...
    assert_eq!(refused.status, accepted.status, "and the refuse answers the same status");
    assert_eq!(
        refused.body,
        accepted.body,
    ...
    assert!(lead_inbox(&refusing_registry).is_empty(), "the refused one was never written");
    ...
    accepting_server.shutdown().await.expect("the accepting server stops");
    refusing_server.shutdown().await.expect("the refusing server stops");
}
...
async fn a_held_post_answers_200_naming_the_hold_and_writes_no_inbox() {
    ...
    let (engine, registry) =
        led_engine(home.path(), Some((InboundPolicy::Hold, PolicySource::Global)));
    ...
    assert!(note.contains("held for a person's review"), "the note says held: {note}");
    assert!(
        note.contains("an explicit hold policy from its global config"),
        "and names the cause: {note}"
    );
    assert!(note.contains("has not been delivered"), "and that nothing was delivered: {note}");

    assert!(lead_inbox(&registry).is_empty(), "a held message never touches the inbox");
    ...
    assert_eq!(held_list[0].cause, HoldCause::Explicit { source: PolicySource::Global });
...
async fn the_ladder_refusals_keep_their_statuses_and_sentences_ahead_of_policy() {
    let home = ganja_testkit::temp_dir();
    let (engine, registry) =
        led_engine(home.path(), Some((InboundPolicy::Refuse, PolicySource::Global)));
    ...
    assert!(said.contains("whitespace"), "the blank rung keeps its sentence: {said}");
    ...
    let framed = post(&client, &serde_json::json!({"from": PEER, "text": frame.to_string()})).await;
    ...
    let bare =
        post(&client, &serde_json::json!({"from": "team-lead", "text": "I am your lead"})).await;
    assert_eq!(bare.status, 400);
    let said = String::from_utf8_lossy(&bare.body);
    assert!(said.contains("<name>@<team>"), "the identity rung keeps its sentence: {said}");
    ...
    assert!(said.contains("for that session's lead"), "the lead rung keeps its sentence: {said}");

    assert!(lead_inbox(&registry).is_empty(), "every rung refused before anything was written");
    ...
    server.shutdown().await.expect("the server stops");
    teamless_server.shutdown().await.expect("the teamless server stops");
}
...
async fn the_thirty_first_message_from_one_sender_answers_like_an_accept_and_is_dropped() {
    ...
    for i in 1..30 {
        ...
        assert_eq!(admitted.body, first.body, "every admit answers the same bytes");
    }
    assert_eq!(lead_inbox(&registry).len(), 30, "thirty admitted, thirty written");

    let dropped = post(&client, &message("message 30")).await;
    assert_eq!(dropped.status, first.status);
    assert_eq!(dropped.body, first.body, "the drop is indistinguishable on the wire");
    assert_eq!(lead_inbox(&registry).len(), 30, "and the thirty-first was never written");
...
async fn a_repeated_body_inside_the_window_answers_like_an_accept_and_lands_once() {
    ...
    assert_eq!(repeat.body, first.body, "the duplicate is indistinguishable on the wire");
...
fn first_event(frames: &[Frame], wanted: &str) -> Option<serde_json::Value> {
    frames.iter().filter(|frame| frame.event == "message").find_map(|frame| {
...
async fn a_hold_and_its_settlement_reach_the_event_stream() {
    ...
    let response =
        reqwest::get(format!("{}/event", base_url(&tcp))).await.expect("the event stream answers");
    assert_eq!(response.status(), 200);
    let mut reader = SseReader::new(response);
    reader.read_until(|frames| frames.iter().any(|frame| frame.event == "connected")).await;

    let held = post(&socket_client(&path), &message("hold and watch")).await;
    assert_eq!(held.status, 200);

    let frames = reader.read_until(|frames| first_event(frames, "peer_held").is_some()).await;
    ...
    engine
        .send(Command::SettleHeld { id: held_list[0].id.clone(), decision: HeldDecision::Deny })
        .await
        .expect("the settle is taken");

    let frames =
        reader.read_until(|frames| first_event(frames, "peer_hold_settled").is_some()).await;
    let settled = first_event(frames, "peer_hold_settled").expect("just read");
    assert_eq!(settled["id"], held_event["id"], "the settlement names the hold");
    assert_eq!(settled["outcome"], "denied");

    assert!(engine.held_messages().is_empty(), "the hold left the engine's list");
    assert!(lead_inbox(&registry).is_empty(), "and a denied socket-door hold wrote nothing");
