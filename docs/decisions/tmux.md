...
| `src/commands/mod.rs` | The declaration mechanism and the register. The `invocations!` macro turns one table entry — Rust name, tmux name, abbreviation, doc, and one line per flag naming its method and argument kind — into a builder, its docs, its `Invocation` impl and its `ENTRIES` row. `Words` is the shared accumulator holding the three set-semantics (`switch`/`value`/`repeat`) and the `--` fence. Every entry also carries its command's flags as data (`Flag`: the letter, and whether a word follows), compacted out of the same table at compile time, so the inventory test can put each one to tmux's own parser. `REGISTRY` and `EXCLUDED` are what the inventory test diffs the running tmux against. |
...
| **Tests and examples** | |
| `tests/inventory.rs` | The coverage gate, in three parts: what the running tmux *prints* against `REGISTRY ∪ EXCLUDED`, which words it *accepts* as commands, and what its parser makes of all 580 declared flags. Needs a `tmux` on `PATH` and hard-fails without one. The first two start no server — `list-commands` is answered by the client alone — while the flag probes need one, because parsing a command line is the server's job; it is this socket's own throwaway server, killed on drop. |
...
The floor is tmux **3.7c** — the release homebrew bottles and CI pours on both platforms. The tables were first written against the next tmux (next-3.8), and what only it takes was **shelved, not deleted**: an `ahead_` prefix on a row keeps the letter, arity, method name and doc in place while generating no method and no served flag, so no builder can spell an argv the floor answers with `unknown flag`; the shelf lands in `Entry::ahead` as data, and serving a row again when the floor moves is deleting the prefix. `switch-mode` is the one whole command typed ahead of the floor — its absence fails loudly, whole and immediately, which is why its rows stay live where a lurking flag would not. An older tmux refuses a flag it does not know, in its own words; a newer one grows commands, and the inventory test names them.

## The inventory contract

Coverage is measured, not claimed. `tests/inventory.rs` asks the tmux on this machine three different questions:
...
2. **Everything claimed is accepted.** For each register entry, `tmux list-commands <word>` must resolve back to exactly the name and abbreviation the register holds. A listing proves tmux *prints* a word; only this proves tmux *accepts* it — which is the whole reason `Entry` carries an alias, since a consumer's config may be written in abbreviations. An **ambiguous** word is a hard failure: that is a claim about this tmux that this tmux contradicts. The summary line splits its count, because only **78** entries are resolved by a genuine abbreviation — the other 14 have none, and resolving those by their full name proves the name and little else.
3. **Every declared flag is a flag, and takes what the builder gives it.** `Entry` carries its command's flags — the letter, and whether a word follows — and each is put to the running tmux's own parser through `bind-key`, which parses a command line and refuses a bad flag *without running it*. tmux's two refusals answer both halves: `unknown flag -X` says the table invented a flag, `-X expects an argument` says a word must follow. All **526** served flags are verified this way. Four of them (`resize-pane`'s directions) earn their argument through a named allowance with a reason per row: the next tmux reads the word after the flag as its optional argument, 3.7 reads it as the positional adjustment beside the flag, and both read it to the same effect — whichever mechanism the running tmux answers with is the one the test records. The **shelf** is put to the same parser: a shelved letter the running tmux serves is reported as arrived (all 55 on next-3.8), one it refuses as not yet (all 55 on 3.7c), and one that demands a word the shelf says it does not take is failed, because then the shelf preserves the wrong knowledge.

Every direction here is **one-way** — installed ⊆ claimed for commands, and version gaps in either direction are reported rather than failed: a register entry an older tmux has never heard of (`switch-mode` on 3.7c), a served flag a newer tmux dropped (`display-panes -b` on next-3.8), a whole command an apt tmux lacks (`new-pane` before 3.7). What *fails* on every version is a claim the running tmux contradicts: an ambiguous abbreviation, an arity the parser disagrees with, a misspelled letter — which on the floor shows up as a refused served flag, and the floor is what CI runs. A letter this tmux documents that neither the rows nor the shelf claim is reported by name, and the report asks the parser which kind it is: today it names `server-access -t`, documented in the usage string and refused by the parser on both sides of the floor, which is the divergence `sessions.rs` argues in prose.
...
- **A new command is one table entry, not a hand-written builder.** Add it to its family's `invocations!` block with a doc line per flag; the macro produces the struct, the methods, their docs, the `Invocation` impl and the `ENTRIES` row. Hand-writing an impl block beside them would break the property that makes `warn(missing_docs)` satisfiable across ~580 generated methods.
- **The sealed leaf is a hard boundary.** The root `depgate.toml`'s `tmux` rule holds all three directions, gated in CI by `cargo depgate check`: `leaf` (this crate's normal tree names no internal crate), `sealed` (no other workspace member consumes `tmux`, over the member set resolved at gate time), and `direct` — the depth-1 allowlist of exactly `futures`, `thiserror` and `tokio`. Do not add an internal edge, a root `tmux` workspace-dependency handle, or a fourth normal external without an explicit user directive superseding P26 and a deliberate `depgate.toml` edit.
...
The inventory run is worth `--no-capture`: all three parts print their counts — `92 installed, 92 typed, 0 awaiting a family`, the resolution split, and `580 declared … 580 verified against this tmux's parser` — and the shrinking half of the first is the coverage measure.
