...
| `cursor-partial-tool-call-probe.txt` | The recording that settles D559 (bead `ganja-code-gzkn`): one user-run turn against `api2.cursor.sh` (2026-09-14, build `ddf8e45`, `ganja.2026-09-15.log:46221-:47100`) — a `bash`, a `write`, then a `read` and a `grep` in one step — read through W1's three debug-line families and nothing else. It settles that cursor sends one `partial_tool_call` per call at the call's start, with empty argument text, carrying the id the exec repeats and no tool name; that two of the four calls crossed a step boundary before their exec; and that cursor's own read-before-write exec came with no update at all. Evidence, not a decoder fixture: nothing reads it, and it is never edited to make a test pass. |
...
| `claude-code-replay-run1.json` | The **derived** artefact a test may read: run 1's twenty-eight inbound frames, lifted from `claude-code-sdk-mcp-probe.txt` under that file's own scrub and nothing else. It exists because the recording is evidence — a test that read the recording could be made green by editing it — while a decoder still has to be pinned against real frames. `src/provider/claude_code/frame_tests.rs` `include_str!`s it and asserts every frame decodes to something this build knows, that the kinds run 1 produced are all present, and that `system/init` at the head of a second turn reads the same as the first. **It under-tests three keys and says so**: the first W2 lane's `assistant` frames omit `timestamp`, `request_id` and `tool_use_meta`, which the CLI does emit, so an extra-field row is pinned by a hand-written test in that same file rather than by this fixture. Re-derive it from the recording rather than editing it; a frame that is here and not there is a frame nobody measured. |
...
- `claude-code-replay-run1.json` is **derived**, never authored: it is a slice of `claude-code-sdk-mcp-probe.txt`'s own `dir: "in"` frames for run 1. The recording is judged by a person and read by no test; this is the half a test reads. If the two ever disagree, the recording wins and this file is re-derived.
- Two crates `include_str!` the `.sse` files: this crate's own unit tests in `src/provider/`, and `ganja-core`'s socket suites, which reach across for them. A rename is a compile error in both, not a silent skip — which is the intent. `cursor-mcp-tools-probe.txt`, `cursor-history-probe.txt`, `cursor-partial-tool-call-probe.txt` and `claude-code-sdk-mcp-probe.txt` are the exceptions and are read by nobody: a probe recording is evidence for a decision, so it is judged by a person and must never be edited to make a test pass.
