...
`insta` snapshots of rendered screens, written by the tests in `../app.rs` and, since P25, two in `../component/chat.rs` — the teammate message and, since **D561**, the folded command expansion. They are the frontend's regression net: a change in layout, wrapping or tool rendering shows up here as a diff instead of going unnoticed.
