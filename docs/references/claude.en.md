...
| [CLI management](https://code.claude.com/docs/en/mcp) | `claude mcp add/list --scope --transport` | ✅ `ganja mcp add/list/get/remove` (D483): entries validated with the loader's own predicates, staged writes to `ganja.json` preserving unknown keys, `ganja.jsonc` edited comment-preservingly (CST; comments, ordering and formatting byte-intact), `get` reports origin and overrides honestly | |
