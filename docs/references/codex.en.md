...
| [`codex mcp add/list/get/remove`](https://developers.openai.com/codex/cli) | CLI management writing `config.toml` | ✅ `ganja mcp add/list/get/remove` (D483): validated staged writes to `ganja.toml` — the same format, since D536 — edited comment-preservingly on toml_edit's CST, a retired `ganja.jsonc`/`ganja.json` in the directory refused with the `ganja config migrate` sentence, `get` reports the origin tier |
