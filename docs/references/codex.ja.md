...
| [`codex mcp add/list/get/remove`](https://developers.openai.com/codex/cli) | `config.toml` を書き換える CLI 管理 | ✅ `ganja mcp add/list/get/remove`(D483): `ganja.json` への検証付き staged 書込み、`ganja.jsonc` は名指し拒否、`get` は由来 tier を報告 |
