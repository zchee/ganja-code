...
- `agent.rs` / `instruction.rs` / `command.rs` — who a turn runs as (build, plan, general, explore, plus whatever the config adds), the system prompt it runs under (a base prompt per model family, the `<env>` block, the `AGENTS.md` family), and the slash commands it can run. A command's template expands the way upstream's does: `` !`cmd` `` runs through the passthrough's spawn at the project root and substitutes its output, and `@file` attaches the way a composer mention would.
