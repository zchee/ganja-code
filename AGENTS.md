...
- `provider/` — `Provider` trait, Anthropic Messages, OpenAI's Responses API and the chat-completions wire grok and Copilot ride, all over a hand-rolled SSE splitter, plus the fake provider — plus `compat`, which is not a fifth wire but the two above reached at an endpoint a config named, under the id it named. Everything filed under `openai` speaks Responses and asks it to seal the model's reasoning, which the transcript carries and the next request hands back; the credential picks the *backend* (the platform, or a ChatGPT seat's) and what the request carries beside the bearer. Two failure channels, never a completed turn. Retry only before the first byte. Credential-travel bounds (no redirects, https-or-loopback) live in `provider/mod.rs`.
